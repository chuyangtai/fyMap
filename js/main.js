+function () {
    'use strict';
    axios.get('https://www.kancloud.cn/yunye/axios/comment',{
        params:{article_id:23484}
    })
    ...
    // 当前挂号、当前就诊
    let chartTab = {dqgh: 0, dqjz: 0}

    //
    let barTab = {week:'周', month:'月'}

    let hospitals = [
        { name: '广元市妇幼保健院', value: [105.879011, 32.432852], count: 0 },
        { name: '广元市第一人民医院', value: [105.829955, 32.432336], count: 0 },
        { name: '广元市中心医院', value: [105.829955, 32.449701], count: 0 },
        { name: '广元市中医院', value: [105.836783, 32.449701], count: 0 },
        { name: '广元市八二一医院', value: [105.824129, 32.452095], count: 0 },
        { name: '广元市第二人民医院', value: [105.83292, 32.438734], count: 0 },
        { name: '广元市第三人民医院', value: [105.804415, 32.426606], count: 0 },
        { name: '广元市第四人民医院', value: [105.801735, 32.385228], count: 0 },
        { name: '广元市利州区中医医院', value: [105.649836, 32.385104], count: 0 },
        ...
        { name: '广元市朝天区人民医院', value: [105.894855, 32.647667], count: 0 },

        { name: '昭化区中医院', value: [105.970605, 32.353732], count: 0 },
        { name: '广元市昭化区人民医院', value: [105.968538, 32.33114], count: 0 },
    ...
    Platforms.prototype.addPatient = function (item, referral, remove) {
        ...
        // map 非指定31家医院就诊患者，直接过滤掉
        let dst = this.findHospital(item.toName);
        if (!dst) return;
        coords.push(dst.value);

        this.option.series[sIndex].data.push({
        ...
        if (remove) {
            this.option.series[sIndex].data[this[rIndex]] = {
                fromName: null,
                toName: null,
                coords: [0, 0]
            };
            this[rIndex]++;
            if (this[rIndex] == 11) {
                this.option.series[sIndex].data = this.option.series[sIndex].data.slice(11);
                this[rIndex] = 0;
            }
        }
    };
    ...
    Platforms.prototype.paysTemplate = function () {
        ...
        return {
            template: '<ul class="pays"><pay v-for="key in list" :pay="key.name" :key="key.name" :val="key.value"></pay></ul>',
            data () {
                return {
                    list: [{ name:'zfb', value:'支付宝'},{name:'wx', value:'微信'},{name:'xyjy', value:'信用就医'},{name:'ydyb', value:'移动医保'},{name:'ylzf', value:'银联支付'}]
                };
            },
            components: {
                pay: li
    ...
    // 地图|患者|转诊
    Platforms.prototype.mapTemplate = function () {
        let pf = this;
        ...
        return {
            ...
            template: `<div class="map">
                            <div class="chart-map" ref="chart"></div>
                            <div class="chart-tab">
            ...
            mounted: function () {
                this.$nextTick(function () {
                    // debugger
                    pf.initMap(this.$refs.chart);
                });
            }
        }
    };
    Platforms.prototype.initMap = function (map) {
        this.mapChart = echarts.init(map);
        this.updateMap();
    };
    Platforms.prototype.setMapOption = function () {
        let series = [];
        ...
        // 实时就诊lines
        series.push(this.drawLines('gy-patient-lines', '#60ff44'));

        // 转诊lines
        series.push(this.drawLines('gy-referral-lines', '#ff0000'));
    ...
    Platforms.prototype.updateMap = function () {
    ...
    // 双向转诊
    Platforms.prototype.setReferrals = function () {
        var rIndex, rItv;
        var _getReferrals = () => {
            this.getReferrals((data) => {
                rIndex = 0;
                // 新数据 直接替换当前
                if (!this.option.series[3].data.length) {
                    rIndex = data.length > 11 ? 11 : data.length;
                    this.addPatients(data.slice(0, rIndex), true);
                }
                if (!rItv) {
                    rItv = setInterval(() => {
                        // 暂停数据填充
                        ...
                        this.addPatient(data[rIndex], true, true);
                        this.updateMap();
                        rIndex++;
                    }, 2500);
                }
        ...
        // 定时获取数据
        _getReferrals();
        setInterval(_getReferrals, 30000);
    };
    ...
    Platforms.prototype.axios = function(response){
        let instance =  axios({
            method: 'post',
            url: response.url,
            data: response.data
    ...
    Platforms.prototype.getPatients = function (callback) {
        let pf = this;
        // pf.axios({
        //     url:'http://localhost:8081/杭州卫生云信息平台/大屏统计监控/广元-v4.0/js/api.json',
        //     data:''
        // }).then(function (response) {
        //     callback (response);
        // })

        axios.get('http://localhost:8081/杭州卫生云信息平台/大屏统计监控/广元-v4.0/js/api.json', {})
          .then(function (response) {
            callback (response);
          })
          .catch(function (error) {
        ...
        //     // pf.axios({
        //     //     url:'/metadata/getCmDomainStandard',
        //     //     data:'data'
        //     // })
        //     setTimeout(()=>{
        //         resolve({
        ...
        //                   { patientName: '张3三', sex: '0', dept: '血液科', fromName: '军师庙火车站', coord: [105.888553, 32.709309], toName: '广元市朝天区人民医院', visitTime: '09:36:09' },
        //                   { patientName: '李3四', sex: '0', dept: '神经内科', fromName: '曾家镇', coord: [106.113761, 32.625418], toName: '广元市朝天区人民医院', visitTime: '09:36:10' },
    ...
    Platforms.prototype.getReferrals = function (callback) {
        setTimeout(() => {
            callback([
                { fromName: '广元市妇幼保健院', toName: '朝天区妇幼保健院' },
                { fromName: '广元市中心医院', toName: '旺苍县中医院' },
                { fromName: '广元市八二一医院', toName: '青川县中医医院' },
                { fromName: '广元市第三人民医院', toName: '剑阁县妇幼保健院' },
                { fromName: '广元市利州区中医医院', toName: '苍溪县中医院' },
                { fromName: '广元市第一人民医院', toName: '苍溪县人民医院' },
                { fromName: '广元市朝天区人民医院', toName: '昭化区中医院' },
                { fromName: '广元市昭化区人民医院', toName: '昭化区妇幼保健院' },
                { fromName: '广元市中医院', toName: '旺苍县人民医院' },
                { fromName: '旺苍县妇幼保健院', toName: '青川县妇幼保健院' },
                { fromName: '广元市第二人民医院', toName: '青川县人民医院' },
                { fromName: '广元市第四人民医院', toName: '剑阁县人民医院' },
                { fromName: '剑阁县中医医院', toName: '苍溪精神卫生' },
                { fromName: '利州区第二人民医院', toName: '苍溪县第二人民医院' },
                { fromName: '朝天区中医医院', toName: '苍溪县妇幼保健院' },
                { fromName: '苍溪皮肤病性病防治院', toName: '广元市第一人民医院' }
            ]);
        }, 10000);
    }
